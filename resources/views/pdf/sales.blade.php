...
<body>
    ...
    <main>
        <table>
            ...
            <tbody>
                ...
                @foreach($sales as $order)
                    ...
                    <?php $CardPaymentSum+=\App\Models\CardPayment::where('sale_id',$order['id'])?\App\Models\CardPayment::where('reversed',\App\Models\CardPayment::IS_NOT_REVERSED)->where('sale_id',$order['id'])->sum('TransactionAmount'):0;?>
                    <?php $CashPaymentSum+=\App\Models\CashPayment::where('sale_id',$order['id'])?\App\Models\CashPayment::where('reversed',\App\Models\CashPayment::IS_NOT_REVERSED)->where('sale_id',$order['id'])->sum('total'):0;?>
                <tr>
        ...
        <div class="project">
            ...
            <div><span>PAYMENTS BY CARD</span> {{number_format($CardPaymentSum,2)}}</div>
            <div><span>PAYMENTS BY CASH</span> {{number_format($CashPaymentSum,2)}}</div>
        </div>
    </main>
    <footer>
        Sales Statement Generated by {{auth()->user()->name}} on {{(Carbon\Carbon::now())->toDayDateTimeString()}}
    </footer>
